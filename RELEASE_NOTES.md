...
### Fixed
- Added `mntpack nuget refresh` as a one-shot workflow to sync and rebuild source-backed packages, clear matching mntpack-local cache entries, and restore the current consumer project.
...
- Source package builds now clone or update GitHub repos, detect the target C# project, run `dotnet restore/build/pack`, and publish `.nupkg` output into the local feed.
...
- New `mntpack nuget ...` commands to ensure `NuGet.config`, add/remove/list package declarations, and apply or restore packages from `mntpack.json`.
...
### Fixed
- Managed `mntpack` installs that still record the legacy owner `MINTILER-DEV/mntpack` can now self-update correctly.
- Reserved-name and protection checks for the managed `mntpack` package now accept both the canonical `mntpack/mntpack` owner and the legacy owner during migration.
...
- Installer release workflow now uses `release_note.md` as the release body.
